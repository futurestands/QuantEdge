...
export const MetricBox = ({ label, value, trend, detail }: any) => (
  <article className="metric-card group hover:border-mint transition-all duration-300 flex flex-col justify-between">
    <div className="flex justify-between items-start">
      ...
      {trend === 'up' ? <ArrowUpRight className="text-mint" size={16} /> : trend === 'down' ? <ArrowDownRight className="text-red-500" size={16} /> : <div />}
    </div>
    <div className="mt-4">
      <strong className="text-3xl tracking-tight leading-none">{value}</strong>
      {detail && <p className="text-[10px] text-slate-500 mt-2 uppercase font-bold tracking-widest">{detail}</p>}
    </div>
  </article>
);
...
export const StatItem = ({ label, value, color }: any) => (
  <div className="space-y-2">
    <span className="text-[10px] uppercase tracking-widest text-slate-500 font-bold block">{label}</span>
    <span className={`text-xl font-bold tracking-tight ${color || "text-white"}`}>{value}</span>
  </div>
);
...
export const ScoreCard = ({ label, score, color, labelOverride }: any) => (
  <div className="panel p-6 bg-ink border-line/40 text-center space-y-3">
     <span className="text-[10px] uppercase font-bold text-slate-500 tracking-widest block">{label}</span>
...
export const ScoreBadge = ({ label, score }: any) => (
  <div className="text-center w-20">
     ...
     <div className={`text-lg font-bold ${score > 80 ? 'text-mint' : score > 60 ? 'text-yellow-500' : 'text-red-500'}`}>{score}</div>
  </div>
);
